...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS GatchaImpact.ActiveQuests
(
...
CREATE TRIGGER UserCredentialsLogInsert
    BEFORE INSERT
    ON GatchaImpact.usercredentials
    FOR EACH ROW
BEGIN
...
CREATE TRIGGER UserCredentialsLogUpdate
    BEFORE UPDATE
    ON GatchaImpact.UserCredentials
    FOR EACH ROW
BEGIN
    INSERT INTO GatchaImpact.UserCredentialsChangeLog(Action, ActionTime, OldUsername, NewUsername, OldPassword,
                                                      NewPassword)
...
CREATE TRIGGER UserCredentialsLogDelete
    BEFORE DELETE
    ON GatchaImpact.UserCredentials
    FOR EACH ROW
BEGIN
